#!/usr/bin/env bash
# E-KIRO-1 runner (design PDN-0004 section 19). Drives one measured kiro-cli
# run through the real v0.5 attempt lifecycle and records what happened.
# Operator tool, not a test: it spends Kiro credits and needs a logged-in
# kiro-cli.
#
#   run.sh <part> <task id> <kind> [--trust <tools>] [--interrupt INT|KILL@<seconds>]
...
#   EK_CARD_DIR  directory holding the kiro-cli card used for the run
#   EK_OPERATOR  who attests stops the probe cannot prove (default: e-kiro-1 operator)
...
out="${EK_OUT:?}"
cards="${EK_CARD_DIR:?}"
operator="${EK_OPERATOR:-e-kiro-1 operator}"
store="$fx/.git/passdown/attempts"
part="${1:?part}" task="${2:?task}" kind="${3:?kind}"
shift 3
trust="" interrupt="" continues="" answer="" resume="" minimal=""
while [ "$#" -gt 0 ]; do
  case "$1" in
...
note() { printf '%s\n' "$*" >>"$log"; }
procs() { # kiro-related processes: pid, ppid, pgid, session, state, command
  ps -A -o pid=,ppid=,pgid=,sess=,stat=,command= | grep -E 'kiro|sleep|seq' | grep -v -E 'grep|run\.sh' || true
}
kiro_pids() { # pid<TAB>start time of every kiro process now running
  ps -A -o pid=,lstart=,command= | grep -i kiro | grep -v -E 'grep|run\.sh' |
...
: >"$log"
note "# E-KIRO-1 part $part — task $task ($kind)"
note ""
note "- date (UTC): $(date -u +%Y-%m-%dT%H:%M:%SZ)"
note "- kiro-cli: $(kiro-cli --version)"
note "- host OS: $(sw_vers -productName 2>/dev/null || uname -s) $(sw_vers -productVersion 2>/dev/null || uname -r)"

args=(--task-ref "docs/plan.md#$task" --planner-repo "$fx" --kind "$kind" --executor kiro-cli --card kiro-cli@1
  --host claude --session e-kiro-1 --place-repo "$fx")
if [ -n "$continues" ]; then
  args+=(--tier external --reason-code review-experiment --reason "E-KIRO-1 continuation" --continues "$continues")
  [ -z "$answer" ] || args+=(--answer "$answer")
else
  args+=(--tier external --reason-code review-experiment --reason "E-KIRO-1 part $part")
...
argv=(kiro-cli chat --output-format stream-json)
[ -z "$trust" ] || argv+=("--trust-tools=$trust")
[ -z "$resume" ] || argv+=(--resume-id "$resume")
argv+=("$(cat "$prompt")")
note "- argv: \`kiro-cli chat --output-format stream-json${trust:+ --trust-tools=$trust}${resume:+ --resume-id $resume} \"<prompt: $part.prompt.md>\"\`"
transport="$store/$id/transport.log"
(cd "$fx" && exec perl -e 'setpgrp(0, 0); exec @ARGV' "${argv[@]}") >"$transport" 2>"$out/$part.stderr" &
wpid=$!
...
note "- kiro processes started during the run and still alive: ${new_kiro:-none}"
note ""
...
if jq -e '.safe_evidence | index("exit+pgroup-empty")' <<<"$facts" >/dev/null; then
  ...
  if [ "$(jq -r .pgroup_members <<<"$facts")" = 0 ] && [ -z "$survivors" ] && [ -z "${new_kiro// /}" ]; then
    ...
    note "- stop: owner-attested by $operator (process group empty, no descendant and no new kiro process alive)"
  else
    note "- stop: NOT attested — processes remain; attempt left unknown"
  fi
fi

# Result: card rule — the last runFinished event's finalText; the payload is
# its last line that is a JSON object.
final="$(jq -r 'select(.type == "runFinished") | .data | "\(.status)\t\(.finalTextTruncated)"' "$transport" 2>/dev/null | tail -n 1)"
session="$(jq -r 'select(.data.sessionId != null) | .data.sessionId' "$transport" 2>/dev/null | head -n 1)"
note "- session id: ${session:-none}"
note "- runFinished (status, truncated): ${final:-none}"
payload="$store/$id/payload.extracted"
jq -r 'select(.type == "runFinished") | .data.finalText' "$transport" 2>/dev/null | tail -n +1 |
  grep -E '^[[:space:]]*\{.*\}[[:space:]]*$' | tail -n 1 >"$payload" || true
if [ -s "$payload" ]; then
...
if [ "$obs" = stopped ]; then
  ...
  verify="$(sed -n "/^- \[ \] $task /,/^- \[/p" "$fx/docs/plan.md" | sed -n 's/.*Verification: `\(.*\)`.*/\1/p' | head -n 1)"
  code_v=0
  ...
  if [ -z "$reason" ]; then
    ...
    H verdict "$id" reject --rev "$(rev "$id")" --reason-code "$reason" --reason "E-KIRO-1 part $part" >/dev/null
    note "- verdict: rejected ($reason); claim $(jq -r .claim.state "$store/$id/receipt.json")"
  fi
fi
note "- git status: \`$(cd "$fx" && git status --porcelain | tr '\n' ';')\`"
note "- tool calls: $(jq -r 'select(.type == "sessionUpdate") | .data.update | select(.sessionUpdate == "tool_call") | .title // .kind // "tool"' "$transport" 2>/dev/null | tr '\n' ';')"
